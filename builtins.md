...
| flicker | `1:minOn?=5`, `2:maxOn?=5`, `3:minOff?=5`, `4:maxOff?=5`, `5:startEnabled?=true` | Flickers on and off periodically. Each tume, stays on for a random number of seconds betweem `minOn` and `maxOn`, then turns off for a random number of seconds between `minOff` and `maxOff`, and repeats this endlessly. `startEnabled` sets whether the trigger should start active. | `flicker 5 7 20 25 yes` - uptime 5-7 seconds, downtime 20-25 seconds, starts enabled |
...
| ---	| ---		| --- 			| --- 			|
| fling | `1:velocity$`, `2:filter$?` | Applies `velocity` to all objects selected by `filter` (filter is treated as a regular expression, defaults to `.*`, which matches everything). `velocity` is treated as a vector: first component is horizontal force (positive is right), second component is vertical force (positive is up). | `fling '0.1;0.8' 'Creature'` Accelerates all creatures slightly to the right and noticeably upwards, `fling '-0.3;0' 'Swarmer'` - drags all neurons (moon's, pebbles' and hunter's) to the left. |
