...
public static partial class VarRegistry {
	...
	internal static void __Clear() {
		__logger.LogDebug("Clear VarRegistry hooks");
		try {
			...
			__logger.LogFatal(__ErrorMessage(site: Site.Clear, message: "Unhandled exception", ex: ex));
		}
	}
	internal static void __Init() {
		__logger.LogDebug("Init VarRegistry hooks");
		try {
			__FillSpecials();
			...
			__logger.LogFatal(__ErrorMessage(Site.Init, "Unhandled exception", ex));
		}
	...
	private static void __TrackCycleLength(
		...
		__logger.DbgVerbose($"Setting $cycletime to {__SpecialVars[SpVar.cycletime]}");
	}
	...
	private static void __WipeSavestate(
		...
		try {
			__logger.LogDebug($"Wiping data for save {save}");
			__EraseData(save);
		}
		catch (Exception ex) {
			__logger.LogError(__ErrorMessage(Site.HookWipe, $"Failed to wipe saveslot {save}", ex));
		}
		orig(self, @char);
	}
	private static void __WipeAll(
		...
		try {
			__logger.LogDebug($"Wiping data for slot {ss}");
			...
			__logger.LogError(__ErrorMessage(Site.HookWipe, $"Failed to wipe all saves for slot {ss}", ex));
		}
		orig(self);
	}
	...
	private static void __ReadPers(
		...
		try {
			...
			if (ss is null) {
				__logger.LogError(__ErrorMessage(Site.HookPersistent, "Could not find current saveslot", null));
				return;
			}
			Save save = MakeSD(ss.Value, __DPSD_Slug[self.GetHashCode()]);
			__logger.LogDebug($"Attempting to load persistent vars for save {save}");
			SerDict? data = __TryReadData(save, DataSection.Persistent);
			if (data is null) __logger.LogDebug("Could not load file, varset will be empty");
			VarsPerSave
				.EnsureAndGet(save, () => new(save))
				._FillFrom(data ?? new(), DataSection.Persistent);
		}
		catch (Exception ex) {
			__logger.LogError(__ErrorMessage(Site.HookPersistent, "Error on read", ex));
		}
		orig(self, s);
	}
	private static string __WritePers(
		...
		try {
			...
			Save save = MakeSD(ss.Value, __DPSD_Slug[self.GetHashCode()]);
			__logger.LogDebug($"Attempting to write persistent vars for {save}");
			...
			__logger.LogError(__ErrorMessage(Site.HookPersistent, "Error on write", ex));
		}
	done:
	...
	private static void __ReadNormal(
		...
		try {
			int? ss = __CurrentSaveslot;
			if (ss is null) {
				__logger.LogError(__ErrorMessage(Site.HookNormal, "Could not find current saveslot", null));
				return;
			}
			Save save = MakeSD(ss.Value, self.saveStateNumber);
			__logger.LogDebug($"Attempting to load non-persistent vars for save {save}");
			...
			__logger.LogError(__ErrorMessage(Site.HookNormal, "Error on read", ex));
		}
	}
	private static string __WriteNormal(
		...
		try {
			...
			__logger.LogError(__ErrorMessage(Site.HookNormal, "Error on write", ex));
		}
	done:
		return orig(self);
	}
	...
	public static Arg GetVar(string name, int saveslot, SlugcatStats.Name character = null!) {
		...
		if (name.StartsWith(PREFIX_GLOBAL)) {
			...
			__logger.LogDebug($"Reading global var {name} for slot {saveslot}");
			return VarsGlobal
				.EnsureAndGet(saveslot, () => {
					__logger.LogDebug("No global record found, creating");
	...
	internal static NamedVars __ReadGlobal(int slot) {
		...
		try {
			...
			__logger.LogError(__ErrorMessage(Site.ReadData, $"Could not read global vars for slot {slot}", ex));
		}
		return res;
	}
	internal static void __WriteGlobal(int slot) {
		...
		try {
			...
			__logger.LogDebug($"Writing global vars for slot {slot}, {fi.FullName}");
			writer.Write(Json.Serialize(dict));
		}
		catch (IO.IOException ex) {
			__logger.LogError(__ErrorMessage(Site.WriteData, $"Could not write global vars for slot {slot}", ex));
		}
	}
	internal static SerDict? __TryReadData(Save save, DataSection section) {
		...
		try {
			...
			__logger.LogError(__ErrorMessage(Site.ReadData, $"error reading {section} for slot {save} ({fi.FullName})", ex));
			return null;
		}
	}
	internal static bool __TryWriteData(Save save, DataSection section, SerDict dict) {
		...
		try {
			...
			__logger.LogError(__ErrorMessage(Site.WriteData, $"error writing {section} for slot {save} ({file.FullName})", ex));
			return false;
		}
	}
	internal static void __EraseData(in Save save) {
		foreach (DataSection sec in Enum.GetValues(typeof(DataSection))) {
			try {
				...
				__logger.LogError(__ErrorMessage(Site.WipeData, $"Error erasing file for {save}", ex));
			}
		}
	}
	#endregion filemanip
